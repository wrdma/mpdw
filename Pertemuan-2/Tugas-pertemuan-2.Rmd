...
# Data
```{r}
data_ces <- import ("C:\\Users\\Rafael Dominico\\Pictures\\Kuliah\\Semester 5\\Metode Peramalan Deret Waktu\\mpdw.xlsx")
head (data_ces)
```
...
```{r}
data_ces.ts <- ts(data_ces$CES9093161101, start = c(1997, 9), frequency = 12)
summary(data_ces.ts)
```
# Plot deret waktu
```{r}
ts.plot(data_ces.ts, xlab="Time Period ", ylab="Employees",
main = "Time Series Plot")
points(data_ces.ts)
dev.copy(png, "plot_data_ces.png")
dev.off()
```
...
# bagi data
training_ma <- data_ces[1:n_train, ]
testing_ma  <- data_ces[(n_train+1):n, ]

# ubah ke time series
train_ma.ts <- ts(
  training_ma$CES9093161101,
  frequency = 12
)

test_ma.ts <- ts(
  testing_ma$CES9093161101,
  frequency = 12
)
```
...
plot(data_ces.ts, col="red",main="Plot semua data")
points(data_ces.ts)
...
```{r}
ggplot() + 
  geom_line(data = training_ma, aes(x = Period, y = CES9093161101, col = "Data Latih")) +
  geom_line(data = testing_ma, aes(x = Period, y = CES9093161101
, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Employees", color = "Legend") +
...
```{r}
data.gab<-cbind(
  aktual=c(data_ces.ts),
  pemulusan=c(data.sma,rep(NA,34)),
...
```{r}
ts.plot(data_ces.ts, xlab="Time Period ", ylab="Employees", main= "SMA N=4 Data Employees")
points(data_ces.ts)
...
error_test.sma = test_ma.ts - data.gab[137:170, 3]
...
data.gab2 <- cbind(aktual = c(data_ces.ts), 
...
```{r}
ts.plot(data_ces.ts, xlab="Time Period ", ylab="Sales", main= "DMA N=4 Data Sales")
points(data_ces.ts)
...
```{r}
#Menghitung nilai keakuratan data uji
error_test.dma = test_ma.ts-data.gab2[137:160,6]
...
# bagi data
training <- data_ces[1:n_train, ]
testing  <- data_ces[(n_train+1):n, ]

# ubah ke time series
train.ts <- ts(training$CES9093161101, frequency = 12)
test.ts  <- ts(testing$CES9093161101, frequency = 12)
```
# Eksplorasi Data
```{r}
plot(data_ces.ts, col="black",main="Plot semua data")
points(data_ces.ts)
```
```{r}
...
ggplot() + 
  geom_line(data = training, aes(x = Period, y = CES9093161101, col = "Data Latih")) +
  geom_line(data = testing, aes(x = Period, y = CES9093161101, col = "Data Uji")) +
  labs(x = "Periode Waktu", y = "Employees", color = "Legend") +
  scale_colour_manual(name="Keterangan:", breaks = c("Data Latih", "Data Uji"),
                      values = c("blue", "red")) + 
  theme_bw() + theme(legend.position = "bottom",
...
#ramalan
ramalanopt<- forecast(HWopt, h=34)
ramalanopt
```
...
resid1<-training$CES9093161101-ramalan1$fitted
head(resid1)
...
resid2<-training$CES9093161101-ramalan1$fitted
head(resid2)
...
# Data aktual
aktual_test <- as.numeric(testing$CES9093161101)

# Forecast 35 periode ke depan
ramalan1_test <- forecast(ses1, h = n_test)
ramalan2_test <- forecast(ses2, h = n_test)
ramalanopt_test <- forecast(HWopt, h = n_test)

# Error (ramalan - aktual)
e1 <- as.numeric(ramalan1_test$mean) - aktual_test
e2 <- as.numeric(ramalan2_test$mean) - aktual_test
eopt <- as.numeric(ramalanopt_test$mean) - aktual_test
...
ramalan1_test <- forecast(ses1, h = nrow(testing))
ramalan2_test <- forecast(ses2, h = nrow(testing))
ramalanopt_test <- forecast(HWopt, h = nrow(testing))

# Akurasi data uji
accuracy(ramalan1_test, testing$CES9093161101)
accuracy(ramalan2_test, testing$CES9093161101)
accuracy(ramalanopt_test, testing$CES9093161101)
```
...
#ramalan
ramalandes1<- forecast(des.1, h=34)
ramalandes1

#Alpha=0.6 dan gamma=0.3
des.2<- HoltWinters(train.ts, gamma = FALSE, beta = 0.3, alpha = 0.6)
plot(des.2)

#ramalan
ramalandes2<- forecast(des.2, h=34)
ramalandes2
```
...
#ramalan
ramalandesopt<- forecast(des.opt, h=34)
ramalandesopt
```
...
) / length(sisaandes1[3:length(train.ts)])
...
selisihdes1 <- as.numeric(ramalandes1$mean) -
               as.numeric(testing$CES9093161101)

SSEtestingdes1 <- sum(selisihdes1^2)
MSEtestingdes1 <- SSEtestingdes1 / length(testing$CES9093161101)
MAPEtestingdes1 <- sum(
  abs(selisihdes1 / testing$CES9093161101) * 100
) / length(testing$CES9093161101)

selisihdes2 <- as.numeric(ramalandes2$mean) -
               as.numeric(testing$CES9093161101)

SSEtestingdes2 <- sum(selisihdes2^2)
MSEtestingdes2 <- SSEtestingdes2 / length(testing$CES9093161101)
MAPEtestingdes2 <- sum(
  abs(selisihdes2 / testing$CES9093161101) * 100
) / length(testing$CES9093161101)

selisihdesopt <- as.numeric(ramalandesopt$mean) -
                 as.numeric(testing$CES9093161101)

SSEtestingdesopt <- sum(selisihdesopt^2)
MSEtestingdesopt <- SSEtestingdesopt / length(testing$CES9093161101)
MAPEtestingdesopt <- sum(
  abs(selisihdesopt / testing$CES9093161101) * 100
) / length(testing$CES9093161101)
...
colnames(akurasitestingdes) <- c(" DES Alpha = 0.2","DES Alpha = 0.7","DES Optimum")
akurasitestingdes
```
# Perbandingan DES dan SES
```{r}
MSEfull <- matrix(
  c(
    MSEtesting1,
...
# Mengubah menjadi data deret waktu
training.ts <- ts(
  training$CES9093161101,
  frequency = 12
)

testing.ts <- ts(
  testing$CES9093161101,
  frequency = 12
)
```
...
```{r}
#Forecast
forecast1 <- predict(winter1, n.ahead = 34)
forecast1.opt <- predict(winter1.opt, n.ahead = 34)
```
...
forecast1_test <- as.numeric(forecast1)[1:n_test]
forecast1_opt_test <- as.numeric(forecast1.opt)[1:n_test]

# Data aktual
aktual_test <- as.numeric(testing$CES9093161101)

# Pastikan panjang forecast dan data aktual sama
forecast1_test <- forecast1_test[1:length(aktual_test)]
forecast1_opt_test <- forecast1_opt_test[1:length(aktual_test)]
...
# MSE
MSEtesting1 <- SSEtesting1 / length(aktual_test)
MSEtesting1.opt <- SSEtesting1.opt / length(aktual_test)
...
# Akurasi data training
SSE2 <- winter2$SSE
MSE2 <- SSE2 / length(training.ts)
RMSE2 <- sqrt(MSE2)

SSE2.opt <- winter2.opt$SSE
MSE2.opt <- SSE2.opt / length(training.ts)
...
# Ambil hasil forecast sebagai numeric vector
forecast2_test <- as.numeric(forecast2)[1:n_test]
forecast2_opt_test <- as.numeric(forecast2.opt)[1:n_test]
...
```
# Kesimpulan
Berdasarkan hasil peramalan uji SMA, DMA, SES, DES, Holt-Winters, metode Double Moving Average (DMA) dengan parameter m=4 memberikan kinerja terbaik dibandingkan metode lainnya. Metode DMA menghasilkan RMSE sebesar 99,23 dan MSE sekitar 9.847,55, yang merupakan nilai terkecil dibandingkan metode SMA, SES, dan DES. Walaupun begitu, metode SES dengan α=0,2 menghasilkan nilai MAPE sedikit lebih kecil, yaitu sebesar 1,076%, dibandingkan DMA sebesar 0,916%. Namun pada data latih, metode SES dengan α=0,7 menunjukkan akurasi terbaik dengan MAPE sebesar 0,247%, yang lebih kecil dibandingkan metode lainnya. Secara keseluruhan, metode DMA dipilih sebagai metode terbaik untuk peramalan jangka pendek karena menghasilkan  MSE paling rendah pada data uji dan mampu menangkap pola tren data dengan lebih baik dibandingkan metode pemulusan tunggal. Hal ini menunjukkan bahwa DMA memberikan hasil peramalan yang paling mendekati data aktual berdasarkan ukuran MSE dan RMSE pada data uji.
